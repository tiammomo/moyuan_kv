...
这条路径可以运行，配置已经统一，但工程化和功能完备度还不如嵌入式路径完整。
...
```text
CLI -c / 默认配置路径
  -> MokvConfig::LoadFromFile(...)
  -> MokvConfig::LoadFromEnv()
  -> raft::ConfigManager
  -> ResourceManager / DBClient
```

其中 `LoadFromFile(...)` 同时兼容旧 `raft.cfg` 和新的 `key=value` 配置格式。

## 当前需要知道的限制

- `data_dir` 现在已经落到 `manifest`、`*.sst` 和 `raft_log_meta`，但服务端路径和嵌入式路径仍然共用同一套本地磁盘布局
- 服务端路径和嵌入式 `DBKVStore` 路径仍然是两套调用链
