...
public:
    ...
    void Put(std::string_view key, std::string_view value) {
        ...
        // 从最高层开始，找到插入位置
        auto p = head_;
        Node* path[32] = {nullptr};  // 记录搜索路径上的节点
        int path_levels[32] = {0};   // 对应的层级
        int path_count = 0;
        ...
        for (int level = static_cast<int>(head_->nexts.size()) - 1; level >= 0; --level) {
            ...
            // 记录路径上的节点（如果需要扩展层级，这些节点的 nexts 可能需要扩展）
            if (level > 0) {
                path[path_count] = p;
                path_levels[path_count] = level;
                ++path_count;
            }
        }
        ...
        if (new_level > head_->nexts.size()) {
            ...
            for (int i = 0; i < path_count; ++i) {
                if (path[i] && path_levels[i] < static_cast<int>(new_level)) {
                    path[i]->nexts.resize(new_level, nullptr);
                }
            }
            // 重新搜索插入位置
            p = head_;
        ...
        if (head_->nexts.size() > node->nexts.size()) {
            node->nexts.resize(head_->nexts.size(), nullptr);
        }
